...
class_names = ['Hazardous waste_Insecticide', 'Hazardous waste_Sphygmometers', 'Hazardous waste_accumulators', 'Hazardous waste_battery', 'Hazardous waste_battery plates',
...
def test_mobilenet():
    train_ds, val_ds, class_names = data_load("D:/project/trash_jpg/trash_jpg_rename", 224, 224, 4)
    print(class_names)
...
def test_cnn():
    train_ds, val_ds, class_names = data_load("D:/project/trash_jpg/trash_jpg_rename", 224, 224, 4)
    model = tf.keras.models.load_model("models/cnn_245_epoch30.h5")
    model.summary()
    loss, accuracy = model.evaluate(val_ds)
